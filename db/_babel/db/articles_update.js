...
var _articles = _interopRequireDefault(require("./schema/articles.js"));
...
function _default(db, model) {
  ...
  _eldeeb.default.log('== articles_index:Start ==');

  if (!model) model = db.model('articles', _articles.default[0]).model;
  /*if (!model) {
    let { tmp } = db.model('articles', schema[0]) //or use mongo.model()
    model = tmp //{model}= ... gives error
  }*/

  var agg = model.aggregate().match({
    status: 'approved',
    $or: [// don't use feild:{$or:[]}, use $or:[{exp1},{exp2},..]
    {
      expireAt: {
        $lt: new Date()
      }
    ...
    location: 1,
    category: 1,
    link: 1 //  week: { $week: '$_id' } //to groupBy week
  ...
  .lookup({
    ...
    category: {
      $first: '$category' //get the first (and the only only) element of [$category] array as a string;  or unwind("category") before group()
  ...
  return agg.exec().then(function () {
    console.log('== articles:start:==');
    agg = model.aggregate().project({
      shortId: 1,
      author: 1,
      title: 1,
      subtitle: 1,
      ...
      _id: '$_id',
      categories: {
        $push: db.implode('$categories.name', ' ')
      },
      shortId: {
        $first: '$shortId'
      },
      ...
      category: '$remove' //shortId: '$remove' //Don't remove shortId because it will be used to access the article (from the index page)

    }).out('tmp.articles');
    agg.exec().then(function () {
      return console.log('== articles:end==');
    }, function (err) {
      return _eldeeb.default.log({
