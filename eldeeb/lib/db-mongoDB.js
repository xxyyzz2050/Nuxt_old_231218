...
connect(options,callback){ //nx: connect(fn(){..}), connect() => use default connection
  let $this=this;
  ...
  return this.run(["connect",options/*,callback*/],function(){
    ...
    let uri=""
     if(options.uri&&!eldeeb.isEmpty(options.uri)){ //don't use options[uri], if so, encodeURIComponent(user & pass)
       uri=options["uri"];
     }else {

         if(typeof options=="string" && options!=""){
           if(options.substr(0,7)!="mongodb")options="mongodb"+(options.srv?"+srv":"")+"://"+options
           uri=options
           options={}

         }else{
           if(options instanceof Array){
               options={user:options[0], pass:options[1], host:options[2], db:options[3]}
             }
    ...
    // https://nodejs.org/api/events.html#events_emitter_once_eventname_listener
     if(callback && typeof callback=="function"){ //or: if(eldeeb.objectType(callback)=="function"); nx: how to get a reference to the current connection i.e: db
       db.on('error',function(err){callback("error",err)}) //connectionError;the listener must be a function
      ...
      //let $this=this,$super=super;
       ...
       db.once('connected',function(){callback("connected",db)})
       db.once('disconnected',function(){callback("disconnected",db)})
       db.once('reconnected',function(){callback("reconnected",db)})
       db.once('connecting',function(){callback("connecting",db)})
       db.once('disconnecting',function(){callback("disconnecting",db)})
       db.once('index',function(){callback("index",db)})
       db.once('close',function(){callback("close",db)})
  ...
  _schema(obj,options){ //this.Schema != super.Schema without this function ; nx: not called by model() if it's name is "Schema"
   let $this=this
    return this.run(["schema",obj],function(){
  ...
  model(coll,schema,options){
    let $this=this
    return this.run(["model",schema,options],function(){
